...
* Premier critère a obtenu la valeur 0 - 20% : Éco2mix est souvent supporté par des mises à jour fonctionnelle, et n'est pas toujours supporté par des mises à jour sécurité. Éco2mix nz sépare pas les deux types de mises à jour, ce qui justifie la valeur attribuée. 
...
* Quatorzième critère a obtenu la valeur L'application Éco2mix a la note 20 - 40% pour l'optimiser le code.

* Quanzième critère a obtenu la valeur L'application Éco2mix est une application mobile et web. Donc, il ne utilise pas le RAM et le Disque Dur. Alors, la note est : 80 - 100%.

* Seizième critère a obtenu la valeur L'application Éco2mix a la note 20 - 40% pour l'optimiser l'utilisation du Mémoire. 

* Dix-septième critère a obtenu la valeur L'application Éco2mix prend l'espace 50MO, alors elle ne prend pas beaucoup d'espace. Pour cela, sa note est : 60 - 80%. 

* Dix-huitième critère a obtenu la valeur L'application Éco2mix est déveloopé en closed-source. Sa note est : 0 - 20%.

* Dix-neuvième critère a obtenu la valeur L'application Éco2mix des Bugs. Sa note est : 50 - 60%.

* Vigntième critère a obtenu la valeur Pour La mauvaise lisibilité du code pour mieux l'appréhender (évolutions, corrections), l'application Éco2mix a la note : 20 - 40%. 
